...
public class FailStateMapper implements StateMapper {
    @Override
    public ObjectNode updateNode(Context context, Trait trait, ObjectNode objectNode) {
        if (!trait.toShapeId().toString().equals("smithy.stateslanguage#failState")) {
            return objectNode;
        }
        return MapperUtils.withStateType("Fail", objectNode);
    }
}
